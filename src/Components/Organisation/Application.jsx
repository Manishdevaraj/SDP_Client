...
function Application() {
  ...
  const ConfrimClick=async()=>
  {
    ...
    await CreateNotification({volunteerid:dbUser?.id,organizerid:applyitem?.organizerid,notifytype:'Pending',notifycontent:"Organizer Still not viwed.",
  ...
  return (
    <>
      <Box className="w-full h-[93vh] flex ">
        {/* Left Section */}
        <Box className="w-8/12 h-full p-4 overflow-auto no-scrollbar">
          ...
          <Grid templateColumns="repeat(3, 1fr)" gap={6} className="w-full overflow-auto">
            {applications?.map((item, index) => (
              <Box
                className="bg-white w-full rounded-3xl mt-1 shadow-2xl"
                key={index}
              >
                <Image
                  src={item?.url}
      ...
      {/* Create Event Form */}
      {isApplication && (
        <Box className="bg-white absolute top-5 w-4/12 ml-96 h-[660px] mt-6 rounded-[50px] shadow-lg p-6 overflow-y-auto no-scrollbar">
      ...
      {isApply&&<Box className="bg-white  absolute top-56 w-5/12 ml-96  mt-6 rounded-[20px] shadow-lg p-4"  >

        <Text className="text-center text-xl font-pbold">{applyitem.eventname}</Text>
        <Box className=" overflow-auto">
        <Text className="">{applyitem.description}</Text>
        </Box>
        <Box className="flex justify-center mt-2">
        <Button colorScheme="green" onClick={ConfrimClick}>Confirm</Button>
        </Box>
