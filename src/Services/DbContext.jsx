...
import { CreatePost, updateAvatar } from "./Api";
...
export function UseContextProvoider({children})
{
 ...
 setisPostSending(false);
     ...
     async function upload_avatar_firbase(file,name,id)
     { 
      
         const storageRef1 = ref(storage, `files/avatar`);
         const storageRef2 = ref(storageRef1, name);
   ...
   () => {
    ...
    getDownloadURL(uploadTask.snapshot.ref).then(async(downloadURL) => {
       console.log('File available at', downloadURL);
        ...
        await updateAvatar(id,downloadURL);
    ...
    return <dbContext.Provider value={{
        isVolunteer,setisVolunteer,isLogin,setIsLogin,user,setuser,google,logout,credentials,SignIn,SignUp,isLoading,setisLoading,setuprecaptcha,dbUser,setdbUser,upload_firbase,isPostSending,setisPostSending,upload_avatar_firbase,upload_brouche_firbase
        }}>
        {children}
    </dbContext.Provider>
}